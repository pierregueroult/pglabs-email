...
export class JmapService {
  ...
  constructor(
    ...
    this.stalwartUrl =
      this.configService.getOrThrow<string>("mail.stalwart.url");
  }
  ...
  private async getSession(userId: string): Promise<{
    ...
    if (user.username) {
      try {
        const accountInfo = await this.stalwartService.getAccount(
          user.username,
        );
        accountDebugInfo = accountInfo;
        this.logger.log(
          `Stalwart Account Info for ${user.username}: ${JSON.stringify(accountInfo)}`,
        );
      } catch (e) {
        this.logger.error(
          `Could not verify Stalwart account for ${user.username}: ${e.message}`,
        );
        accountDebugInfo = { error: e.message };
      }
    } else {
      this.logger.warn(
        `User ${userId} has no username, skipping Stalwart account check.`,
      );
    ...
    this.logger.log(
      `Requesting JMAP session for ${user.email} from ${this.stalwartUrl}/.well-known/jmap`,
    );
  ...
  private async executeJmapRequest(
    ...
    this.logger.debug(
      `Executing JMAP request to ${apiUrl} (Session reported: ${sessionData.session.apiUrl})`,
    );
    // this.logger.debug(`JMAP Request Body: ${localRequestBody}`);

    try {
      const response = await lastValueFrom(
        this.httpService.post<JmapResponse>(apiUrl, localRequestBody, {
          headers: {
  ...
  async getEmails(
      ...
      methodCalls: [
        [
          "Email/query",
          {
            accountId: accountId,
            filter:
              Object.keys(queryFilter).length > 0 ? queryFilter : undefined,
    ...
    if (!queryMethodResponse || queryMethodResponse[1].error) {
      throw new Error(
        "Failed to query emails: " + JSON.stringify(queryMethodResponse?.[1]),
      );
    }
  ...
  async sendEmail(
    ...
    const { session, accountId, authHeader, email } =
    ...
    if (!identity) {
      ...
      this.logger.warn(
        `No identity found for email ${email}, using first available identity: ${identities[0].id}`,
      );
    }
        ...
        [
          "Email/set",
          {
            ...
            create: {
              draft: {
                from: [
                  { email: email, name: identity?.name || "Pierre Guéroult" },
                ],
                to: [{ email: to }],
                subject: subject,
    ...
    if (!creationMethodResponse || creationMethodResponse[1].notCreated) {
      throw new Error(
        `Failed to create email draft: ${JSON.stringify(creationMethodResponse?.[1])}`,
      );
    }
    ...
    if (!submissionMethodResponse || submissionMethodResponse[1].notCreated) {
      throw new Error(
        `Failed to submit email: ${JSON.stringify(submissionMethodResponse?.[1])}`,
      );
    }
      ...
      methodCalls: [
        [
          "Email/set",
          {
            ...
            update: {
              [emailId]: {
                "keywords/$seen": isSeen ? true : null,
              },
            },
          },
          "setEmailSeen",
  ...
  async moveEmail(
    ...
    // This effectively moves it from wherever it was to the new mailbox.
    const request: JmapRequest = {
      using: [URN_JMAP_CORE, URN_JMAP_MAIL],
      methodCalls: [
        [
  ...
  /**
   * Deletes an email (Moves to Trash if not there, or destroys if already in Trash).
   */
  async deleteEmail(userId: string, emailId: string): Promise<void> {
    // 1. Get Mailboxes to find Trash
    const mailboxes = await this.getMailboxes(userId);
    ...
    // 2. Get Email to check current mailboxes
    const { session, accountId, authHeader } = await this.getSession(userId);

    // Fetch the email's current mailboxIds
    const getRequest: JmapRequest = {
      using: [URN_JMAP_CORE, URN_JMAP_MAIL],
      methodCalls: [
        [
    ...
    if (isInTrash) {
      // Permanently destroy
      const destroyRequest: JmapRequest = {
        using: [URN_JMAP_CORE, URN_JMAP_MAIL],
        methodCalls: [
          [
            "Email/set",
